...
object HotItems {
  def main(args: Array[String]): Unit = {
    ...
    //    val testPath = ""
    //    val dataStream = env.readTextFile(testPath)
...
class TopNHostItems(topSize: Int) extends KeyedProcessFunction[Long, ItemViewCount, String] {
  ...
  override def onTimer(timestamp: Long, ctx: KeyedProcessFunction[Long, ItemViewCount, String]#OnTimerContext, out: Collector[String]): Unit = {
    ...
    val allTtems: ListBuffer[ItemViewCount] = new ListBuffer[ItemViewCount]()
    import scala.collection.JavaConversions._
    for (item <- itemState.get()) {
      allTtems += item
    }
    //按照count大小排序,并取前N个
    val sortedItems = allTtems.sortBy(_.count)(Ordering.Long.reverse).take(topSize)
